    ...
    <>
      ...
      <div ref={sidebarRef} className={`fixed top-0 left-0 w-64 h-full bg-white z-20 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300`}>
        <ul className="flex flex-col items-center space-y-8 mt-8 py-20">
          ...
          <li className="flex justify-evenly space-x-4 w-full px-4">
            <Link to="/login" className="text-black font-light">Account</Link>
            <Link to="/login" className="text-black font-light">Wishlist</Link>
          </li>
          <li
            className="relative"
          ...
          >
            <button className="flex items-center text-black font-light">
              Collections
              <BsFillTriangleFill
                className={`ml-2 md:ml-4 mt-1 transform transition-transform w-2 h-2 ${isSidebarDropdownOpen ? 'rotate-90' : 'rotate-180'}`}
              />
            </button>
      ...
      <nav className="hidden md:flex fixed top-0 left-0 right-0 z-10 items-center justify-between px-4 py-4 md:px-12 md:py-8">
        <div className="flex items-center space-x-4 md:space-x-24">
          ...
          <Link to="/login" className={`font-light ${textColor}`}>Account</Link>
        </div>
